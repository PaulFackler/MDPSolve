% Burr3mom2param Computes the Burr-3 parameters fromthe mean, CV and skewness  
% USAGE
%   [a,b,c]=Burr3mom2param(mu,cv,gamma);
%
% This is the inverse of Burr3mom
% see: Burr3mom
function [a,b,c]=Burr3mom2param(mu,cv,gamma)
...
%c=ifthenelse(c<=3, 3+5*rand(1,1), c);
...
res = [gammaln(b+2./c) + gammaln(1-2./c) +   gb1 -   logb - 2*(gbc1 + gc1);
       gammaln(b+3./c) + gammaln(1-3./c) + 2*gb1 - 2*logb - 3*(gbc1 + gc1)] - eta;
if nargout>1
  pb1  = psi(b+1);
  pc1  = psi(1-1./c);
  pbc1 = psi(b+1./c);
  ...
  pc3  = psi(1-3./c);
  pbc3 = psi(b+3./c);
  J = [(pbc2 +   pb1 - 2*pbc1)*b-1  2*((pc2 - pbc2) - (pc1 - pbc1))/c;
       (pbc3 + 2*pb1 - 3*pbc1)*b-2  3*((pc3 - pbc3) - (pc1 - pbc1))/c];
end
