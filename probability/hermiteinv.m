% HERMITEINV Generates a Hermite interpolating function for an inverse function
% USAGE
...
while j~=0
  ...
  if dy<=mindy || xmid==x(j) || xmid==xi
    ...
    if n>length(ind)
      ind = [ind;zeros(chunk,1)]; %#ok<AGROW>
      y   = [y  ;zeros(chunk,1)]; %#ok<AGROW>
      x   = [x  ;zeros(chunk,1)]; %#ok<AGROW>
      d   = [d  ;zeros(chunk,1)]; %#ok<AGROW>
    end
    % update links
    ind(i) = n;
    ind(n) = j;
